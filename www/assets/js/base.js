...
goBackButton.forEach(function (el) {
    el.addEventListener("click", function () {
        if (PurpleHeart.Animation.goBack) {
            goBackAnimation();
        }
        else {
            window.history.go(-1);
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("PurpleHeartAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("PurpleHeartAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("PurpleHeartDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (PurpleHeart.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (PurpleHeart.Dark_Mode.local_mode.enable) {
    var nightStart = PurpleHeart.Dark_Mode.local_mode.start_time;
    var nightEnd = PurpleHeart.Dark_Mode.local_mode.end_time;
...
if (PurpleHeart.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("PurpleHeartDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("PurpleHeartDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("PurpleHeartDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
if (document.querySelector(".offcanvas") === null) {
    ...
    var CookiesStatus = localStorage.getItem("PurpleHeartCookiesStatus")
    ...
    document.querySelectorAll(".accept-cookies").forEach(function (el) {
        el.addEventListener("click", function () {
            localStorage.setItem("PurpleHeartCookiesStatus", "1")
        })
    })
}
...
function testMode() {
    ...
    console.clear();
    console.log("%cPurpleHeart", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(PurpleHeart.PWA.enable, "PWA")
    testModeMsg(PurpleHeart.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(PurpleHeart.Dark_Mode.local_mode.enable, "Local dark mode (between " + PurpleHeart.Dark_Mode.local_mode.start_time + ":00 and " + PurpleHeart.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(PurpleHeart.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(PurpleHeart.RTL.enable, "RTL")
    testModeMsg(PurpleHeart.Test.enable, "Test mode")
    testModeMsg(PurpleHeart.Test.alert, "Test mode alert")
    ...
    testModeMsg(PurpleHeart.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = PurpleHeart.Test.word;
    var value = "";
